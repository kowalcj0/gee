...
function runcleanup() {
    ...
    for i in ${!hosts[@]} ; do
        cLog "killing server agent on ${hosts[$i]}... "
        ( ssh -nq -o StrictHostKeyChecking=no \
        ...
        && {
            cLog "Result files were successfully zipped on remote host: ${hosts[$i]}"
            cLog "Downloading zipped results from: ${host[$i]}"
            scp -q -C -o UserKnownHostsFile=/dev/null \
                                         -o StrictHostKeyChecking=no \
                                         -i $PEM_PATH/$PEM_FILE \
                                         -P $REMOTE_PORT \
    ...
    if ${cfgTailRemoteJmeterLogs} ; then
        for i in ${!hosts[@]} ; do
            if [ -e ${LOCAL_HOME}/projects/${project}/${DATETIME}-${hosts[$i]}-jmeter.out ] ; then
                ...
                cLog "-"
                cLog "-"
                cLog "- Tailing last 20 lines of remote JMeter log: ${LOCAL_HOME}/projects/${project}/${DATETIME}-${hosts[$i]}-jmeter.out"
                cLog "-"
                cLog "-"
                # set nice color for the output :)
                tput setaf 6; tput bold;
                tail -n20 ${LOCAL_HOME}/projects/${project}/${DATETIME}-${hosts[$i]}-jmeter.out
    ...
    # These result files are downloaded from remote nodes from:
    # $REMOTE_HOME/$project-$DATETIME-*.jtl
    # They are created by the "Generate Summary Results" listener
    #
    # Bear in mind that they don't have the header line, and they contain almost 
    # the same data as the result files generated by the jp@gc report listeners
    # The only differences is that $project-$DATETIME-*.jtl does contains 
    # an additionatl field "thread name" but it's missing "IdleTime" field.
    # Acctually we could get rid of that step and use combined result file:
    # ${DATETIME}-results-complete.jtl
    # created a bit later to calculate required values
    #
    ...
    if [ ! -z "$DB_HOST" ] ; then
        ...
        cLog "Step 2': Moving $LOCAL_HOME/projects/$project/$project-$DATETIME-sorted.jtl to $LOCAL_HOME/projects/$project/$project-$DATETIME-appended.jtl"
        mv $LOCAL_HOME/projects/$project/$project-$DATETIME-sorted.jtl $LOCAL_HOME/projects/$project/$project-$DATETIME-appended.jtl
    fi
    #
	cLog "Step 3: Removing blank lines"
    ...
    #
	cLog "Step 4: Remove any lines containing \"0,0,Error:\" - which seems to be an intermittant bug in JM where the getTimestamp call fails with a nullpointer"
    ...
    #
	cLog "Step 5: Calclulating test duration"
    ...
    if ${cfgCreateMergedResultFile} ; then
        cLog "Merging result files is enabled"
        #
        cLog "Step 1: Extracting zipped result files"
        for (( i=0; i<$instance_count; i++ )) ; do
            # retry unzippin results file if it takes too long
            CMD="unzip -o $LOCAL_HOME/projects/$project/$DATETIME-$i-jtls.zip -d $LOCAL_HOME/projects/$project/$i"
        ...
        #
        cLog "Step 2: Grouping all result files"
        for (( i=0; i<$instance_count; i++ )) ; do
        ...
        #
        cLog "Step 3: Sorting ${DATETIME}-results-grouped.jtl > ${DATETIME}-results-sorted.jtl"
        sort -u $LOCAL_HOME/projects/$project/${DATETIME}-results-grouped.jtl > $LOCAL_HOME/projects/$project/${DATETIME}-results-sorted.jtl
        #
        cLog "Step 4: Removing blank lines from ${DATETIME}-results-sorted.jtl > ${DATETIME}-results-noblanks.jtl"
        sed '/^$/d' $LOCAL_HOME/projects/$project/${DATETIME}-results-sorted.jtl  > $LOCAL_HOME/projects/$project/${DATETIME}-results-noblanks.jtl
        #
        cLog "Step 5: Removing lines with some intermittant errors from ${DATETIME}-results-noblanks.jtl > ${DATETIME}-results-noErrors.jtl"
        ...
        #
        cLog "Step 6: Moving last line to the beggining from ${DATETIME}-results-noErrors.jtl > ${DATETIME}-results-complete.jtl"
        sed '1h;1d;$!H;$!d;G' $LOCAL_HOME/projects/$project/${DATETIME}-results-noErrors.jtl  > $LOCAL_HOME/projects/$project/${DATETIME}-results-complete.jtl \
            && {
    ...
    if ${cfgCreateMergedFileForJenkinsPerfPlugin} ; then
        cLog "Merging all jenkins.xml files is enabled"
    ...
    if ${cfgGenerateAggregatedResponseTimePercentilesReports} ; then
        ...
        CMD="java -Djava.awt.headless=true -jar $LOCAL_HOME/${JMETER_VERSION}/lib/ext/CMDRunner.jar --loglevel WARN --tool Reporter --generate-csv $LOCAL_HOME/projects/$project/aggregatePercentiles.tmp --input-jtl $LOCAL_HOME/projects/$project/${DATETIME}-results-complete.jtl --plugin-type ResponseTimesPercentiles"
    ...
    if ${cfgCreateAggregateCSVReport} ; then
        ...
        CMD="java -Djava.awt.headless=true -jar $LOCAL_HOME/${JMETER_VERSION}/lib/ext/CMDRunner.jar --loglevel WARN --tool Reporter --generate-csv $LOCAL_HOME/projects/$project/aggregate.tmp --input-jtl $LOCAL_HOME/projects/$project/${DATETIME}-results-complete.jtl --plugin-type AggregateReport"
        repeatTillSucceedWithExecTimeout 5 ${cfgGraphGenerationTimeout} "${CMD}" \
        && { 
    ...
    if ! ${cfgCreateHTMLReport} ; then
        ...
        if  [ -d $LOCAL_HOME/${JMETER_VERSION} ] && [[ -n $(ls $LOCAL_HOME/${JMETER_VERSION}/) ]] ; then
            ...
            for res in "${results[@]}"; do
                ...
                CMD="java -Djava.awt.headless=true -jar $LOCAL_HOME/${JMETER_VERSION}/lib/ext/CMDRunner.jar --loglevel WARN --tool Reporter ${insertRelativeTimeParam} --generate-png $LOCAL_HOME/projects/$project/results/${cfgHtmlReportGraphsDir}/${res}.png --input-jtl $LOCAL_HOME/projects/$project/${DATETIME}-results-complete.jtl --plugin-type ${res} --width ${cfgReportGraphWidth} --height ${cfgReportGraphHeight}"
                repeatTillSucceedWithExecTimeout 5 ${cfgGraphGenerationTimeout} "${CMD}" \
                && {
